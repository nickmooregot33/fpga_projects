...
void WritePacket(HANDLE h, int address, const void *data, size_t data_size) {
	byte buf[(3 + 64) * 256]; /*67*256 = 17,152 = 0x4300*/
...
/*too stupid to even say what arguments he's expecting.  This is ridiculously stupid*/
/*argv1 must be an NES rom*/
int main(int argc, char * argv[]) {
	/*confirm program is running*/
	char pause_string[150];
	printf("Hey, the UART program is running\n");

	/*open a com port*/
	HANDLE h = CreateFile(L"\\\\.\\COM4", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (!h) {
		printf("CreateFile failed\n");
		scanf("%s", pause_string);
		return 0;
	}
	...
	printf("we have apparently got the serial port configured?");
	scanf("%s", pause_string);
	
	
	/*open the given file, must be a NES rom*/
	FILE *f = fopen(argv[1], "rb");
	if (!f) { /*if f is a nullptr*/
		printf("File open fail\n"); return 1; 
	}

	{ 
		char v = 1; 
		WritePacket(h, 0x35, &v, 1); 
	}
	{ 
		char v = 0; 
		WritePacket(h, 0x35, &v, 1); 
	...
	size_t total_read = 0xffffff;//2^24 - 1 = 16MB but he wrote 10180;
	size_t pos = 0;

	/*transferring the file across the uart*/
  
	while (pos < total_read) {
		char buf[16384];/*2^14 = 16KB*/
	...
	int last_keys = -1;
	scanf("%s", pause_string);
	/*managing the usb controller to fpga communication*/
	for(;;) {
		JOYINFOEX joy;
